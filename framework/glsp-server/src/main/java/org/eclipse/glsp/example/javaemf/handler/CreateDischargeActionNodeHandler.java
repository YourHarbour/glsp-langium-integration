...
public class CreateDischargeActionNodeHandler extends EMFCreateOperationHandler<CreateNodeOperation> {
    ...
    public CreateDischargeActionNodeHandler() {
        super(HealthcareModelTypes.DISCHARGE_ACTION);
    }
    ...
    @Override
    public String getLabel() { return "Discharge Action"; }
    ...
    protected Command createActionAndShape(final Optional<GPoint> relativeLocation) {
        ...
        DischargeAction newAction = createAction();
        Command actionCommand = AddCommand.create(editingDomain, actionCard,
            HealthcareDiagramPackage.Literals.ACTION_CARD__DISCHARGE_ACTIONS, newAction);
    ...
    protected DischargeAction createAction() {
        DischargeAction newAction = HealthcareDiagramFactory.eINSTANCE.createDischargeAction();
        newAction.setId(UUID.randomUUID().toString());
        setInitialName(newAction);
        return newAction;
    }

    protected void setInitialName(final DischargeAction action) {
