...
public class CreateTestNodeHandler extends EMFCreateOperationHandler<CreateNodeOperation> {
    ...
    public CreateTestNodeHandler() {
        super(HealthcareModelTypes.TEST);
    }
    ...
    protected Command createActionAndShape(final Optional<GPoint> relativeLocation) {
        ...
        Command actionCommand = AddCommand.create(editingDomain, actionCard,
            HealthcareDiagramPackage.Literals.ACTION_CARD__TESTS, newTest);

        Shape shape = createShape(idGenerator.getOrCreateId(newTest), relativeLocation);
    ...
    protected Test createTest() {
        Test newTest = HealthcareDiagramFactory.eINSTANCE.createTest();
        newTest.setId(UUID.randomUUID().toString());
        setInitialName(newTest);
        return newTest;
    }

    protected void setInitialName(final Test test) {
        Function<Integer, String> nameProvider = i -> "New" + test.eClass().getName() + i;
